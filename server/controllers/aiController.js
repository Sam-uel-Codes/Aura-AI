...
export const removeImageBackground = async (req, res) => {
  try {
    ...
    await sql`INSERT INTO creations (user_id, prompt, content, type) VALUES(${userId}, 'Removing the background image', ${secure_url}, 'image')`;
...
export const resumeReview = async (req, res) => {
  try {
    ...
    if (resume.size > 5 * 1024 * 1024) {
      return res.json({
        success: false,
        message: "The file size exceeds 5 MB.",
      });
    }
    ...
    const prompt = `Review the following resume and provide constructive feedback on its strength, weakness and areas of improvement. Content: \n\n ${pdfData.text}`;
